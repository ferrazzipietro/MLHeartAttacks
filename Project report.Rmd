...
data_surv_tot <- tot %>%
  ...
  add_column( Age_Ejection.Fraction = tot$Age * train$Ejection.Fraction) %>%
...
data_surv_test <- data_surv_tot[-train.idxs,]
data_surv_tot <- data_surv_tot[train.idxs,]
```
...
set.seed(1234)
desc_inner <- makeResampleInstance("RepCV", reps = 2,  folds = 5, task = classif_task)
```
...
library(parallelMap)
parallelStartSocket(6) # start in socket mode and create 2 processes on localhost
...
ps <- makeModelMultiplexerParamSet(
  ...
  classif.neuralnet = makeParamSet(makeIntegerVectorParam("hidden", len = 1, lower = 5, upper = 10),
                                   makeNumericParam("threshold", lower = 1, upper = 5, trafo = function(x) {10^x}),
...
rpart.plot(rpart_model, under = TRUE, box.palette = "BuBn", branch.lty = 3, tweak = 1.1)
```
